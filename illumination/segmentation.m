...
showsegments('img_evaltests/dehazed.png',4);
...
%     for n = 1:divisions_vertical
%         gradient(segments(:,:,:,n),0);
%     end
    for n = 1:divisions_vertical
        [segments_processed(:,:,:,n) detail(n)] = subtractillumination(segments(:,:,:,n),0);
        subplot(plot_nrows,divisions_vertical,n+divisions_vertical),imshow(segments_processed(:,:,:,n),[0 255]),title(sprintf('Detail level: %i', detail(n)));
    end;
