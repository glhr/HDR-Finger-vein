...
for i = 2:0.2:3 
    ...
    for j=1:numel(windows)
        ...
        [img, output, pattern] = miura_usage(strcat('img_evaltests/',dataset,'/hdr_',window,'_movmean.png'),4000,6,9,sigma,1);
        % figure(1)
        % subplot(3,1,1)
        %   imshow(img,[])
        ...
        imwrite(pattern,strcat('img_evaltests/',dataset,'/maxcurve_',window,'_',num2str(sigma),'.png'));
