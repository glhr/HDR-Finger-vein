...
%% Adjusting LEDs
  ...
  lowest_pwm = zeros([8 1]);
  highest_pwm = [1024 1024 1024 1024 1024 1024 1024 1024];
  pwm_step_size = 10;
  max_pwm = 1023;
  iter = 60;
  maxcount = zeros([8 1]);
    ...
    while ~done
        ...
        fprintf('\nThreshold: %i\t | MGI: %.2f\t\t | STD %.2f\t\t', grey_thr(j), mgi(j), stds(j));
           ...
           if mgi(j) < (grey_thr(j)-10)
                ...
                pwm(j) = floor(lowest_pwm(j) + (1024 - lowest_pwm(j))/2);
                lowest_pwm(j) = pwm(j);
                if pwm(j) >= max_pwm
                    pwm(j) = max_pwm;
           ...
           elseif mgi(j) > (grey_thr(j)+10)
                ...
                %pwm(j) = pwm(j) - pwm_step_size;
                pwm(j) = floor(highest_pwm(j) - (highest_pwm(j) - lowest_pwm(j))/2);
                highest_pwm(j) = pwm(j);
                
                if pwm(j) <= 0
                    pwm(j) = 0;
                    fprintf(' (setting PWM to min)');
                    mincount(j) = mincount(j) + 1;
                end
                setLed(LED_driver, j+7, pwm(j));
                pause(1/200);
           else
